...
export const checkAuth = (...authRoles: Role[]) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if(!sessionToken){
      throw new AppError(status.UNAUTHORIZED, "Unauthorized Access! No session token is provided!");
    }
    else{
      ...
      if(sessionExist && sessionExist.user){
        ...
        if(user.isDeleted){
          throw new AppError(status.UNAUTHORIZED, "Unauthorize access! User is deleted!");
        }

        if(authRoles.length > 0 && !authRoles.includes(user.role)){
          throw new AppError(status.FORBIDDEN, "You do not have the permission to access this resource")
        }
      }
